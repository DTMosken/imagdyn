...
模块：[`imagdyn/wind.py`](../imagdyn/wind.py)。默认数值集中在 [`imagdyn/params.py`](../imagdyn/params.py)。
...
2. **行星气压带**（经度均分 **36** 扇区）：
   - 扇区内平均气温 → 热赤道；
   - 哈德利环流模拟，高空瑞利摩擦角动量路径积分 + 热成风 `u_crit` → 副热带高气压带；
   - 副高极侧 `|dT̄/dφ|` 最大 → 副极地低气压带；
   - 极地高压固定 **±89°**；
   - 相邻气压带之间用**余弦半波**连接异常；
   - 扇区结果经度**周期插值**复合；meta 记平均位置与 `upper_amc`。
3. 局地热力气压异常（赤道附近因**弱温度梯度近似**减弱）+ 陆地对高压异常的动态阻尼。
4. 2D 扩散（经度环绕；东西向核宽随 `1/cos(φ)` 放大）；**87°→89°** 渐消东西向异常。
5. ∇p → 科里奥利 + **二次方**地表阻力平衡；UV 邻域卷积。
6. 地形阻挡 / 分流 / 背风（水体海拔按 0；极地 `|lat|≥89` 强制海洋）。
